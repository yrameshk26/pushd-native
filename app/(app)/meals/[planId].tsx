...
export default function MealPlanDetailScreen() {
  ...
  const logMealMutation = useMutation({
    ...
    onSuccess: () => {
      Alert.alert('Logged', 'Meal added to today\'s food log.');
    },
    onError: () => {
    ...
    (meal: MealPlanMeal) => {
      Alert.alert(
        ...
        [
          ...
          {
            text: 'Log',
            onPress: () => logMealMutation.mutate({ mealId: meal.id }),
          },
        ]
      );
    },
  ...
  const handleViewGroceryList = useCallback(() => {
    router.push(`/(app)/meals/grocery-list?planId=${planId}` as any);
  }, [router, planId]);
        ...
        <View style={styles.summaryCard}>
          ...
          <View style={styles.summaryRow}>
            <SummaryChip
              ...
              label={`${plan.totalDays} day${plan.totalDays !== 1 ? 's' : ''}`}
            />
            <SummaryChip
              icon="flame-outline"
              label={`~${plan.calorieTarget > 0 ? plan.calorieTarget : avgDailyCalories} kcal/day`}
              accent
            />
          </View>
