...
type ProgramGoal = 'strength' | 'hypertrophy' | 'endurance' | 'weight_loss' | 'athletic_performance';
type DurationWeeks = 4 | 6 | 8 | 12;
...
const GOALS: { value: ProgramGoal; label: string; icon: string; desc: string }[] = [
  ...
  { value: 'endurance', label: 'Endurance', icon: 'heart-outline', desc: 'Improve stamina and cardiovascular fitness' },
  { value: 'weight_loss', label: 'Weight Loss', icon: 'flame-outline', desc: 'Burn fat while preserving muscle' },
  { value: 'athletic_performance', label: 'Athletic Performance', icon: 'trophy-outline', desc: 'Sport-specific power and agility' },
];
...
const SESSION_OPTIONS: { value: SessionMinutes; label: string }[] = [
...
function Step2({
  ...
  return (
    <ScrollView showsVerticalScrollIndicator={false}>
      ...
      <Text style={[stepStyles.sectionLabel, { marginTop: 24 }]}>Session Duration</Text>
      <View style={stepStyles.chipRow}>
        {SESSION_OPTIONS.map(({ value, label }) => {
          const selected = form.sessionMinutes === value;
          return (
            <TouchableOpacity
              key={value}
              style={[stepStyles.chip, selected && stepStyles.chipSelected]}
              onPress={() => update({ sessionMinutes: value })}
              activeOpacity={0.7}
            >
              <Text style={[stepStyles.chipText, selected && stepStyles.chipTextSelected]}>
                {label}
...
export default function GenerateProgramScreen() {
  ...
  function canNext(): boolean {
    ...
    if (step === 1) return form.daysPerWeek > 0 && form.sessionMinutes > 0;
  ...
  const generateMutation = useMutation({
    mutationFn: async () => {
      ...
      return data as GeneratedProgram & { id?: string };
    },
    onSuccess: (data) => {
    ...
    onError: (err: any) => {
      const message = err?.response?.data?.error ?? 'Failed to generate program. Please try again.';
      setError(message);
      setGenerating(false);
    },
  });
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      if (newId) {
        setTimeout(() => router.push(`/(app)/programs/${newId}` as any), 800);
      }
    },
    onError: (err: any) => {
      setIsSaving(false);
