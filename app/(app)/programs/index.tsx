...
function ProgramCard({ item }: { item: ProgramListItem }) {
  ...
  return (
    <TouchableOpacity
      ...
      onPress={() => router.push(`/(app)/programs/${item.id}` as any)}
      activeOpacity={0.7}
    >
...
export default function ProgramsScreen() {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <TouchableOpacity
        style={styles.aiCard}
        onPress={() => router.push('/(app)/ai-planner')}
        activeOpacity={0.8}
      >
        ...
        <View style={styles.aiText}>
          <Text style={styles.aiTitle}>Build a custom program</Text>
          <Text style={styles.aiDesc}>
            Get a personalised program based on your goals, schedule and equipment
          </Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  aiText: { flex: 1 },
  aiTitle: { color: '#fff', fontSize: 14, fontWeight: '700', marginBottom: 2 },
  aiDesc: { color: '#888', fontSize: 12, lineHeight: 17 },
  aiBadge: { flexDirection: 'row', alignItems: 'center', gap: 3 },
  aiBadgeText: { color: '#6C63FF', fontSize: 12, fontWeight: '700' },
